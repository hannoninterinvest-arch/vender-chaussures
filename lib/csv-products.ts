...
export function parseProductCsv(text: string): { products: CsvProduct[]; errors: CsvParseError[] } {
  ...
  lines.slice(1).forEach((line, i) => {
    ...
    try {
      ...
      const weightRaw = Number(get("weightGrams").replace(",", "."));
      products.push({
        name,
        ...
        images,
        weightGrams: Number.isFinite(weightRaw) && weightRaw >= 1 ? Math.round(weightRaw) : 900,
      });
    } catch (err) {
      errors.push({
        line: lineNo,
...
export const CSV_TEMPLATE = `nom;marque;prix;promo;achat;description;genre;categorie;nouveau;couleurs;pointures;images;poids
Oxford Noir;ELVARO;489;399;280;Richelieu cuir lustré;homme;ceremonie;oui;Noir:#141210@/chaussures/oxford-noir.jpg|Cognac:#B5763A@/chaussures/oxford-cognac.jpg;40|41|42|43|44;/chaussures/oxford-noir.jpg|/chaussures/oxford-cognac.jpg;950
Derby Cognac;ELVARO;459;;260;Derby ville en cuir;homme;ville;oui;Cognac:#8B5A2B@/chaussures/derby-cognac.jpg;40|41|42|43;/chaussures/derby-cognac.jpg|/chaussures/derby-tabac.jpg;900
`;
